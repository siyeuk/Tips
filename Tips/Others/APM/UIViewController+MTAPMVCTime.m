//
//  UIViewController+MTAPMVCTime.m
//  MT_Tips
//
//  Created by lss on 2022/10/12.
//

#import "UIViewController+MTAPMVCTime.h"
#import <objc/runtime.h>

static char const kMTFakeKVORemoverKey;  //关联的对象Key
static char const kMTVCBeginDateKey; //vc开始加载时间的key

static NSString *const kMTFakeKeyPath = @"MT_FakeKeyPath";  //假冒的被观察属性key


///冒充KVO观察者，是为了生成观察目标的KVO子类
@interface MTFakeKVOObserver : NSObject
@end
@implementation MTFakeKVOObserver
+ (instancetype)shared {
...
///负责移除冒充的KVO观察者
@interface MTFakeKVORemover : NSObject
@property (nonatomic, weak) id target;  //被观察的目标
@end
@implementation MTFakeKVORemover
- (void)dealloc {
    [_target removeObserver:[MTFakeKVOObserver shared] forKeyPath:kMTFakeKeyPath];
}
@end

@implementation UIViewController (MTAPMVCTime)

+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        Class class = [UIViewController class];
        if (![NSStringFromClass(class) hasPrefix:@"MT"]) {
            //仅仅测自己创建的VC
            return;
        }
        ///hook vc的初始化方法
...
- (void)createAndHookKVOClass {
    ...
    [self addObserver:[MTFakeKVOObserver shared] forKeyPath:kMTFakeKeyPath options:NSKeyValueObservingOptionNew context:nil];
    
    //保存观察目标VC，当VC实例释放时，移除KVO
    MTFakeKVORemover *remover = [[MTFakeKVORemover alloc] init];
    remover.target = self;
    objc_setAssociatedObject(self, &kMTFakeKVORemoverKey, remover, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
static void apm_loadView(UIViewController *kvo_self, SEL _sel) {
    ...
    //记录开始加载的时间
    objc_setAssociatedObject(kvo_self, &kMTVCBeginDateKey, [NSDate date], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
static void apm_viewDidAppear(UIViewController *kvo_self, SEL _sel, BOOL animated) {
    ...
    NSDate *beginDate = objc_getAssociatedObject(kvo_self,  &kMTVCBeginDateKey);
    if (beginDate) {
        //计算方法耗时
    ...
    //重置记录的开始时间
    objc_setAssociatedObject(kvo_self, &kMTVCBeginDateKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
