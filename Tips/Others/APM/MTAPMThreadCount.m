...
@implementation MTAPMThreadCount
...
+ (void)startMonitorThreadCount {
    ...
    kk_dispatch_main_async_safe(^{
        taskName = [MTTimer execTask:self selector:@selector(clearThreadCountIncrease) start:0 interval:1.0 repeats:YES async:NO];
    });
}
//定时器每一秒都将线程增长数置0
