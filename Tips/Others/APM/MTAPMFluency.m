...
@interface MTAPMRunLoop : NSObject{
...
static void runLoopObserverCallBack(CFRunLoopObserverRef observer, CFRunLoopActivity activity, void *info) {
    ...
    MTAPMRunLoop *monitor = (__bridge MTAPMRunLoop*)info;
    // 记录状态值
...
- (CADisplayLink *)displayLink {
    if (!_displayLink) {
        _displayLink = [CADisplayLink displayLinkWithTarget:[MTProxy proxyWithTarget:self] selector:@selector(displayLinkTick:)];
...
@interface MTAPMFluency ()
@property (nonatomic, strong) MTAPMFps *fps;
@property (nonatomic, strong) MTAPMRunLoop *runLoop;
@end
@implementation MTAPMFluency
...
- (nonnull id)copyWithZone:(nullable NSZone *)zone {
    return [MTAPMFluency sharedInstance];
}

#pragma mark - Public
+ (instancetype)sharedInstance {
    static MTAPMFluency *luency = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        luency = [[super allocWithZone:NULL] init];
        luency.type =  MTAPMFluencyTypeRunloop;
    });
    return luency;
}
...
- (void)startMonitorFluency {
    ...
    if (self.type == MTAPMFluencyTypeRunloop) {
    ...
    }else  if (self.type == MTAPMFluencyTypeFps) {
        [self.fps play];
    ...
    }else if (self.type == MTAPMFluencyTypeAll) {
        [self.fps play];
        [self.runLoop startRunning];
        self.fps.fpsChanged = ^(float fps) {
            if ([weakSelf.delegate respondsToSelector:@selector(APMFluency:didChangedFps:callStackOfStuck:)]) {
...
- (MTAPMFps *)fps {
    if (!_fps) {
        _fps = [[MTAPMFps alloc] init];
    }
    return _fps;
}
- (MTAPMRunLoop *)runLoop {
    if (!_runLoop) {
        _runLoop = [[MTAPMRunLoop alloc] init];
    }
    return _runLoop;;
}
