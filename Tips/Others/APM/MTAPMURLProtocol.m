...
///计算请求报文的大小   即上行流量
@interface NSURLRequest (MTDataLength)
@end
@implementation NSURLRequest (MTDataLength)
- (NSUInteger)mt_getLineLength {
...
- (NSUInteger)mt_getHeadersLengthWithCookie {
    NSUInteger headersLength = 0;
    
    NSDictionary<NSString *, NSString *> *headerFields = self.allHTTPHeaderFields;
    NSDictionary<NSString *, NSString *> *cookiesHeader = [self mt_getCookies];
...
- (NSDictionary<NSString *, NSString *> *)mt_getCookies {
...
@interface MTAPMURLProtocol ()<NSURLSessionTaskDelegate, NSURLSessionDataDelegate>
...
@implementation MTAPMURLProtocol


#pragma mark - Public
///开始监听网络
+ (void)startMonitorNetwork {
    [NSURLProtocol registerClass:[MTAPMURLProtocol class]];
}
///结束监听网络
+ (void)stopMonitorNetwork {
    [NSURLProtocol unregisterClass:[MTAPMURLProtocol class]];
}
...
- (void)stopLoading {
    ...
    NSUInteger sendLength = [self.request mt_getLineLength] + [self.request mt_getBodyLength] + [self.request mt_getHeadersLengthWithCookie];
