 ...
 */
@interface MTUnusedResourceC ()

@property (nonatomic, strong) NSMutableArray *dataSource;

@end

@implementation MTUnusedResourceC
...
- (void)getUnusedResourceData {
    ...
    //文件目录路径
    NSString *folderPath = @"/Users/lss/Desktop/MT_Tips";
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        for (MTResourceInfo *imgInfo in imgResources) {
            //其他文件中是否使用了该图片
            BOOL isUsed = NO;
            for (MTResourceInfo *fileInfo in files) {
                //文件内容
        ...
        for (MTResourceInfo *unusedImg in unusedImgs) {
...
- (NSMutableArray *)searchAllUnderFolderPath:(NSString *)searchPath fileTypes:(NSString *)suffixs regularExpression:(NSString *)expression {
    NSError *error;
    NSArray *filesArray = [[NSFileManager defaultManager] subpathsOfDirectoryAtPath:searchPath error:&error] ;
    ...
    while (subPath = [filesEnumerator nextObject]) {
        @autoreleasepool {
            ...
            MTResourceInfo *info = [[MTResourceInfo alloc] init];
...
- (nonnull UITableViewCell *)tableView:(nonnull UITableView *)tableView cellForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ...
    MTResourceInfo *info = self.dataSource[indexPath.row];
