...
@interface MTImage () {
    ...
    MTImageDecoder * _imageDecoder;  //解码工具
    NSArray <MTImageFrame *> * _preloadedFrames;  //预加载的图片帧
}
...
+ (MTImage *)imageNamed:(NSString *)name{
    ...
    NSArray *scales = [MTImage preferredScales];
...
+ (MTImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        _imageDecoder = [[MTImageDecoder alloc] init];
...
- (void)setPreloadAllAnimatedImageFrames:(BOOL)preloadAllAnimatedImageFrames{
    ...
    if (_preloadAllAnimatedImageFrames && _imageDecoder.frameCount > 0) {
        ...
        for (NSUInteger i = 0, max = _imageDecoder.frameCount; i < max; i++) {
            MTImageFrame *imageFrame = [_imageDecoder imageFrameAtIndex:i];
            if (imageFrame) {
...
- (MTImageFrame *)imageFrameAtIndex:(NSInteger)index {
    ...
    dispatch_semaphore_wait(_preloadedLock, DISPATCH_TIME_FOREVER);
    MTImageFrame *imageFrame = _preloadedFrames[index];
    dispatch_semaphore_signal(_preloadedLock);
