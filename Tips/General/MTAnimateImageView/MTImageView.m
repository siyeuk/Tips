...
#define MT_LOCK_VIEW(...) dispatch_semaphore_wait(view->_lock, DISPATCH_TIME_FOREVER); \
__VA_ARGS__; \
dispatch_semaphore_signal(view->_lock);

#define MT_LOCK(...) dispatch_semaphore_wait(self->_lock, DISPATCH_TIME_FOREVER); \
__VA_ARGS__; \
dispatch_semaphore_signal(self->_lock);

#define MT_BUFFER_SIZE (10 * 1024 * 1024) // 10MB (minimum memory buffer size)
#define MT_CLAMP(_x_, _low_, _high_)  (((_x_) > (_high_)) ? (_high_) : (((_x_) < (_low_)) ? (_low_) : (_x_)))

#pragma mark - 弱引用对象

//临时弱引用对象，解决循环引用的问题  引自 YYWeakProxy
@interface MTWeakProxy : NSProxy <NSObject>
@property (nullable, nonatomic, weak, readonly) id target;
@end
@implementation MTWeakProxy
+ (instancetype)proxyWithTarget:(id)target {
    return [[MTWeakProxy alloc] initWithTarget:target];
}
...
@interface MTImageView () {
    ...
    MTImage                 *_curAnimatedImage; /// 当前动画的image
...
#pragma mark - 解码操作 -
//解码获取某帧image的线程操作
@interface MTImageFrameDecodeOperation : NSOperation
@property (nonatomic, weak)   MTImageView   *view;
@property (nonatomic, assign) NSUInteger    nextIndex; //解码下一帧
@property (nonatomic, strong) MTImage       *curImage;
@end
@implementation MTImageFrameDecodeOperation
- (void)main {
    __strong MTImageView *view = _view;
    if (!view) return;
    ...
    for (int i = 0; i < max; i++, idx++) {
        @autoreleasepool {
            ...
            __strong MTImageView *view = _view;
            if (!view) break;
            MT_LOCK_VIEW(BOOL miss = (view->_buffer[@(idx)] == nil));
            if (miss) {
                MTImageFrame *imageFrame = [_curImage imageFrameAtIndex:idx];
                if ([self isCancelled]) break;
                MT_LOCK_VIEW(view->_buffer[@(idx)] = imageFrame ? imageFrame : [NSNull null]);
                view = nil;
            }
        }
    }
...
- (void)resetAnimated {
    dispatch_once(&_onceToken, ^{
        ...
        self->_displayLink = [CADisplayLink displayLinkWithTarget:[MTWeakProxy proxyWithTarget:self] selector:@selector(playAnimationImage:)];
...
- (void)playAnimationImage:(CADisplayLink *)displayLink {
    ...
    MTImageFrame *nextBufferedImage = nil;
    MTImageFrame *curBufferedImage = nil;
    ...
    if (!bufferIsFull && _requestQueue.operationCount == 0) {
        //异步串行队列去执行下一帧的解码任务
        MTImageFrameDecodeOperation *operation = [MTImageFrameDecodeOperation new];
        operation.view = self;
...
- (void)calcMaxBufferCount {
    ...
    max = MAX(max, MT_BUFFER_SIZE);
    if (_maxBufferSize) max = max > _maxBufferSize ? _maxBufferSize : max;
    double maxBufferCount = (double)max / (double)bytes;
    maxBufferCount = MT_CLAMP(maxBufferCount, 1, 100);
...
- (void)setImage:(MTImage *)image{
    if ([image isMemberOfClass:[UIImage class]]) {
        image = [MTImage imageWithData:UIImagePNGRepresentation(image)];
    }
    [self resetAnimated];
    _curAnimatedImage = (MTImage *)image;
