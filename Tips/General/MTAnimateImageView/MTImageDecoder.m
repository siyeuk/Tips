...
//来源YYImage
#define MT_FOUR_CC(c1,c2,c3,c4) ((uint32_t)(((c4) << 24) | ((c3) << 16) | ((c2) << 8) | (c1)))
#define MT_TWO_CC(c1,c2) ((uint16_t)(((c2) << 8) | (c1)))

@implementation MTImageFrame @end

@interface MTImageDecoder () {
...
NSInteger imageDataType(NSData * data){
    ...
    if (!data) return MTImageTypeUnknown;
    uint64_t length = CFDataGetLength(dataRef);
    if (length < 16) return MTImageTypeUnknown;
    
    const char *bytes = (char *)CFDataGetBytePtr(dataRef);
    
    uint32_t magic4 = *((uint32_t *)bytes);
    switch (magic4) {
        case MT_FOUR_CC(0x4D, 0x4D, 0x00, 0x2A): { // big endian TIFF
            return MTImageTypeTIFF;
        } break;
            
        case MT_FOUR_CC(0x49, 0x49, 0x2A, 0x00): { // little endian TIFF
            return MTImageTypeTIFF;
        } break;
            
        case MT_FOUR_CC(0x00, 0x00, 0x01, 0x00): { // ICO
            return MTImageTypeICO;
        } break;
            
        case MT_FOUR_CC(0x00, 0x00, 0x02, 0x00): { // CUR
            return MTImageTypeICO;
        } break;
            
        case MT_FOUR_CC('i', 'c', 'n', 's'): { // ICNS
            return MTImageTypeICNS;
        } break;
            
        case MT_FOUR_CC('G', 'I', 'F', '8'): { // GIF
            return MTImageTypeGIF;
        } break;
            
        case MT_FOUR_CC(0x89, 'P', 'N', 'G'): {  // PNG
            uint32_t tmp = *((uint32_t *)(bytes + 4));
            if (tmp == MT_FOUR_CC('\r', '\n', 0x1A, '\n')) {
                return MTImageTypePNG;
            }
        } break;
            
        case MT_FOUR_CC('R', 'I', 'F', 'F'): { // WebP
            uint32_t tmp = *((uint32_t *)(bytes + 8));
            if (tmp == MT_FOUR_CC('W', 'E', 'B', 'P')) {
                return MTImageTypeWebP;
            }
        } break;
    }
    ...
    switch (magic2) {
        ...
        case MT_TWO_CC('P', 'I'):
        case MT_TWO_CC('C', 'I'):
        case MT_TWO_CC('C', 'P'): { // BMP
            return MTImageTypeBMP;
        }
        case MT_TWO_CC(0xFF, 0x4F): { // JPEG2000
            return MTImageTypeJPEG2000;
        }
    }
    
    // JPG             FF D8 FF
    if (memcmp(bytes,"\377\330\377",3) == 0) return MTImageTypeJPEG;
    // JP2
    if (memcmp(bytes + 4, "\152\120\040\040\015", 5) == 0) return MTImageTypeJPEG2000;
    
    return MTImageTypeUnknown;
};
...
- (MTImageFrame *)imageFrameAtIndex:(NSInteger)index{
    ...
    MTImageFrame * imageFrame = [[MTImageFrame alloc] init];
