...
NSString* const MTkeychainService = @"com.lss.ios.keychain";
static NSString* const keychainErrorDomain = @"com.lss.ios.keychain.errorDomain";
...
+ (NSError *)errorWithErrorCode:(OSStatus)errorCode {
    ...
    switch (errorCode) {
        ...
        case errSecDuplicateItem: // -25299
            errorMsg = NSLocalizedString(@"指定的项在keychain中已经存在。", nil);
            break;
        case errSecItemNotFound: // -25300
            errorMsg = NSLocalizedString(@"在钥匙链中找不到指定的项。 ", nil);
            break;
        default: {
