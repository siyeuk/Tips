...
@implementation MTAVPlayer

- (void)seek:(CMTime)time{
    [self.avPlayer seekToTime:time toleranceBefore:CMTimeMake(1, 1) toleranceAfter:CMTimeMake(1, 1) completionHandler:^(BOOL finished) {
        
    }];
}

+ (instancetype)sharedAVPlayer {
    static MTAVPlayer *avPlayer = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        avPlayer = [[MTAVPlayer alloc] init];
    });
    return avPlayer;
}
