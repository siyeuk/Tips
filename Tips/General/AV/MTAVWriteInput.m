...
@property (nonatomic, assign) MTAVWriterFileType                    outputFileType; //写入输出文件类型 默认 MTAVWriterFileTypeVideo


@end

@implementation MTAVWriteInput
...
- (void)setOutputFileURL:(NSURL *)outputFileURL {
    ...
    if (self.outputFileType == MTAVWriterFileTypeAudio) {
        _assetWriter = [AVAssetWriter assetWriterWithURL:outputFileURL fileType:AVFileTypeAC3 error:nil];
    } else if (self.outputFileType == MTAVWriterFileTypeVideo || self.outputFileType == MTAVWriterFileTypeSilentVideo) {
...
- (void)startWritingToOutputFileAtPath:(NSString *)path fileType:(MTAVWriterFileType)fileType deviceOrientation:(UIDeviceOrientation)deviceOrientation{
    ...
    if (self.outputFileType == MTAVWriterFileTypeSilentVideo || self.outputFileType == MTAVWriterFileTypeVideo) {
    ...
    if (self.outputFileType == MTAVWriterFileTypeAudio || self.outputFileType == MTAVWriterFileTypeVideo){
    ...
    if (filterImage) {
        ...
        @autoreleasepool {
            @synchronized(self) {
                ...
                if (!self.isStartWriting  && (self.outputFileType == MTAVWriterFileTypeVideo || self.outputFileType == MTAVWriterFileTypeSilentVideo)) {
                    //一个写入周期内只能执行一次
...
- (void)writingAudioSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    @autoreleasepool {
        @synchronized(self) {
            if (!self.isStartWriting && self.outputFileType == MTAVWriterFileTypeAudio) {
                [self.assetWriter startWriting];
                [self.assetWriter startSessionAtSourceTime:CMSampleBufferGetPresentationTimeStamp(sampleBuffer)];
                self.isStartWriting = YES;
            }
