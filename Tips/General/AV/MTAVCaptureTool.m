...
@interface MTAVCaptureTool ()<AVCapturePhotoCaptureDelegate, AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
...
@property (nonatomic, assign) MTAVCaptureType               avCaptureType; //音视频捕获类型 默认 MTAVCaptureTypeAv
...
@implementation MTAVCaptureTool


+ (instancetype)sharedAvCaptureTool {
    static MTAVCaptureTool *avCaptureTool = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        avCaptureTool = [[MTAVCaptureTool alloc] init];
    });
...
- (void)setOutputFileURL:(NSURL *)outputFileURL {
    ...
    if (self.avCaptureType == MTAVCaptureTypeAudio) {
        _assetWriter = [AVAssetWriter assetWriterWithURL:outputFileURL fileType:AVFileTypeAC3 error:nil];
    } else if (self.avCaptureType == MTAVCaptureTypeVideo || self.avCaptureType == MTAVCaptureTypeAv) {
...
//开始录制视频
- (void)startRecordVideoToOutputFileAtPath:(NSString *)path recordType:(MTAVCaptureType)avCaptureType{
    ...
    //这个API 每次开始录制时设置视频输出方向，会造成摄像头的短暂黑暗，有问题的代码可以看此类文件夹下的MTAVCaptureTool-bug副本；切换摄像头时设置此属性没有较大的影响
    // captureConnection.videoOrientation = AVCaptureVideoOrientationPortrait;
    ...
    if ([self.assetWriter canAddInput:self.assetWriterAudioInput] && self.avCaptureType == MTAVCaptureTypeAv) {
...
- (void)startRecordAudioToOutputFileAtPath:(NSString *)path {
    self.avCaptureType = MTAVCaptureTypeAudio;
    //移除重复文件
    if ([[NSFileManager defaultManager] fileExistsAtPath:path]) {
        [[NSFileManager defaultManager] removeItemAtPath:path error:nil];
    }
...
- (void)writerVideoSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    @autoreleasepool {
        @synchronized(self) {
            if (!self.canWrite && (self.avCaptureType == MTAVCaptureTypeAv || self.avCaptureType == MTAVCaptureTypeVideo)) {
...
- (void)writerAudioSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    @autoreleasepool {
        @synchronized(self) {
            if (!self.canWrite && self.avCaptureType == MTAVCaptureTypeAudio) {
                [self.assetWriter startWriting];
                [self.assetWriter startSessionAtSourceTime:CMSampleBufferGetPresentationTimeStamp(sampleBuffer)];
                self.canWrite = YES;
            }
            if (self.assetWriterAudioInput.readyForMoreMediaData && self.canWrite) {
                ...
                //                        }else if (self.avCaptureType == MTAVCaptureTypeAv || self.avCaptureType == MTAVCaptureTypeVideo) {
                //                            [self stopRecordVideo];
                //                        }
                //                    }
                //                }
