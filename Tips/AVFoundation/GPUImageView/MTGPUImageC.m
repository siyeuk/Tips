...
#import "MTBlurView.h"
#import "MTShotFocusV.h"
#import "MTWaterMarkC.h"

#define KMaxDurationOfVideo  15.0 //录制最大时长 s
#define KRecordVideoFilePath  [NSTemporaryDirectory() stringByAppendingString:@"GPUImageViewVideo.mp4"]  //视频录制输出地址

@interface MTGPUImageC ()<GPUImageVideoCameraDelegate>{
...
@property (nonatomic, strong) MTBlurView *shotBtn; //拍摄按钮
...
@property (nonatomic, strong) MTShotFocusV *focusView;   //当前聚焦视图
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self focusAtPoint:CGPointMake(MT_kScreenWidth/2.0, MT_kScreenHeight/2.0)];
    //监听设备方向，旋转切换摄像头按钮
...
- (void)viewDidDisappear:(BOOL)animated {
    ...
    [MTDelayPerform mt_cancelDelayPerform];
}
...
- (GPUImageView *)captureView {
    if (!_captureView) {
        _captureView = [[GPUImageView alloc] initWithFrame:CGRectMake(0, 0, MT_kScreenWidth, MT_kScreenHeight)];
...
- (GPUImageMovieWriter *)movieWriter {
    if (!_movieWriter) {
        ...
        _movieWriter = [[GPUImageMovieWriter alloc] initWithMovieURL:[NSURL fileURLWithPath:KRecordVideoFilePath] size:CGSizeMake((MT_kScreenWidth - (int)MT_kScreenWidth%16)*[UIScreen mainScreen].scale, (MT_kScreenHeight - (int)MT_kScreenHeight%16)*[UIScreen mainScreen].scale)];
...
- (UIButton *)backBtn {
    if (_backBtn == nil) {
        ...
        _backBtn.center = CGPointMake((self.view.mt_width/2 - 70/2.0)/2.0, self.view.mt_height - 80);
...
- (UIView *)shotBtn {
    if (_shotBtn == nil) {
        _shotBtn = [[MTBlurView alloc] init];
        _shotBtn.userInteractionEnabled = YES;
        _shotBtn.frame = CGRectMake(0, 0, 70, 70);
        _shotBtn.center = CGPointMake(self.view.mt_width/2.0, self.view.mt_height - 80);
        _shotBtn.clipsToBounds = YES;
        _shotBtn.layer.cornerRadius = _shotBtn.mt_width/2.0;
        //轻触拍照，长按摄像
        ...
        self.whiteView.center = CGPointMake(_shotBtn.mt_width/2.0, _shotBtn.mt_height/2.0);
...
- (UIButton *)switchCameraBtn {
    if (_switchCameraBtn == nil) {
        _switchCameraBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.mt_width - 30 - 30, 44 , 30, 30)];
...
- (MTShotFocusV *)focusView {
    if (_focusView == nil) {
        _focusView= [[MTShotFocusV alloc] initWithFrame:CGRectMake(0, 0, 60, 60)];
    }
    return _focusView;
}
- (UILabel *)tipsLabel {
    if (_tipsLabel == nil) {
        _tipsLabel = [[UILabel alloc] initWithFrame:CGRectMake((self.view.mt_width - 140)/2.0, self.shotBtn.mt_y - 20 - 30, 140, 20)];
...
- (void)endRecord {
    ...
    _movieWriter = nil;
    if ([[NSFileManager defaultManager] fileExistsAtPath:KRecordVideoFilePath]) {
        MTWaterMarkC * waterMarkController = [[MTWaterMarkC alloc] init];
        ...
        [self presentViewController:waterMarkController animated:NO completion:^{
         ...
         [MTAlertView showAlertViewWithText:@"录制失败" delayHid:1];
    }
}
// 添加水印
- (void)addWatermark {
    ...
    // 水印层
    UIView *watermarkView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.mt_width, self.view.mt_height)];
    watermarkView.backgroundColor = [UIColor clearColor];
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 60, 80)];
    label.text = @"水印层水印层\n边录制边添加水印";
    label.font = [UIFont systemFontOfSize:30];
    [label sizeToFit];
    label.center = CGPointMake(MT_kScreenWidth/2.0, 88);
    ...
    [self.filter setFrameProcessingCompletionBlock:^(GPUImageOutput *output, CMTime time) {
        MT_DISPATCH_ON_MAIN_THREAD(^{
            label.mt_y = label.mt_y < MT_kScreenHeight ? label.mt_y + 5 : label.mt_y - 5;
        })
...
- (void)tapFocusing:(UITapGestureRecognizer *)tap {
    ...
    if(point.y > self.shotBtn.mt_y || point.y < self.switchCameraBtn.mt_y + self.switchCameraBtn.mt_height) {
        return;
    }
...
- (void)recordVideo:(UILongPressGestureRecognizer *)longPress {
    switch (longPress.state) {
        case UIGestureRecognizerStateBegan:{
            self.shotBtn.mt_size = CGSizeMake(100, 100);
            self.shotBtn.center = CGPointMake(self.view.mt_width/2.0, self.view.mt_height - 80);
            self.shotBtn.layer.cornerRadius =  self.shotBtn.mt_height/2.0;
            self.whiteView.mt_size = CGSizeMake(40, 40);
            self.whiteView.center = CGPointMake(self.shotBtn.mt_width/2.0, self.shotBtn.mt_height/2.0);
            self.whiteView.layer.cornerRadius = self.whiteView.mt_width/2.0;
            //开始计时
            [self startTimer];
            //添加进度条
        ...
        case UIGestureRecognizerStateEnded:{
            self.shotBtn.mt_size = CGSizeMake(70, 70);
            self.shotBtn.center = CGPointMake(self.view.mt_width/2.0, self.view.mt_height - 80);
            self.shotBtn.layer.cornerRadius =  self.shotBtn.mt_height/2.0;
            self.whiteView.mt_size = CGSizeMake(50, 50);
            self.whiteView.center = CGPointMake(self.shotBtn.mt_width/2.0, self.shotBtn.mt_height/2.0);
            self.whiteView.layer.cornerRadius = self.whiteView.mt_width/2.0;
            //取消计时器
            dispatch_source_cancel(self->_gcdTimer);
