...
@interface MTWaterMarkC ()<GPUImageMovieWriterDelegate, MTAVPlayerDelegate>
...
@property (nonatomic, strong) MTBlurView *addWatermark; //添加水印  文本和GIF
...
- (GPUImageMovieWriter *)movieWriter {
    if (!_movieWriter) {
        ...
        //视频写入
        _movieWriter = [[GPUImageMovieWriter alloc] initWithMovieURL:[NSURL fileURLWithPath:KWatermarkVideoFilePath] size:CGSizeMake((MT_kScreenWidth - (int)MT_kScreenWidth%16)*[UIScreen mainScreen].scale, (MT_kScreenHeight - (int)MT_kScreenHeight%16)*[UIScreen mainScreen].scale)];
        _movieWriter.shouldPassthroughAudio = YES;//是否使用源音源
        _movieWriter.delegate = self;
    }
    return _movieWriter;
}
- (MTBlurView *)addWatermark {
    if (_addWatermark == nil) {
        _addWatermark = [[MTBlurView alloc] initWithFrame:CGRectMake(0, 0, 70, 70)];
        _addWatermark.center = CGPointMake(self.view.mt_width/2.0, self.view.mt_height - 80);
        _addWatermark.layer.cornerRadius = _addWatermark.mt_width/2.0;
...
- (UIActivityIndicatorView *)activityIndicatorView {
    if (!_activityIndicatorView) {
        ...
        _activityIndicatorView.center = CGPointMake(MT_kScreenWidth/2.0, MT_kScreenHeight/2.0);
    }
...
- (void)addWatermarkClicked:(id)sender {
    ...
    // 水印层
    UIView *watermarkView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, MT_kScreenWidth, MT_kScreenHeight)];
    watermarkView.backgroundColor = [UIColor clearColor];
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 200, self.view.mt_width, 80)];
    label.text = @"这是一个测试版本的水印";
    label.font = [UIFont systemFontOfSize:24];
    label.numberOfLines = 0;
    label.textAlignment = NSTextAlignmentCenter;
    label.textColor = [UIColor redColor];
    MTImageView *imageView = [[MTImageView alloc] initWithFrame:CGRectMake(0, 88, 100, 100)];
    imageView.mt_centerX = self.view.mt_width/2.0;
    ...
    MTImage *image = [MTImage imageWithContentsOfFile:imagePath];
...
- (void)movieRecordingCompleted {
    ...
    NSLog(@"add水印成功");
    MT_DISPATCH_ON_MAIN_THREAD(^{
        [MTAlertView showAlertViewWithText:@"add水印成功" delayHid:1];
...
- (void)movieRecordingFailedWithError:(NSError *)error {
    NSLog(@"add水印失败");
    MT_DISPATCH_ON_MAIN_THREAD(^{
        [MTAlertView showAlertViewWithText:@"add水印失败" delayHid:1];
        [self.activityIndicatorView stopAnimating];
        [self.activityIndicatorView removeFromSuperview];
    });
}
#pragma mark - MTAVPlayerDelegate
//播放完成
