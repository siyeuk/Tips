...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        _isNeedLine = YES;
        _photoframeLineW = 6;
        _backAreaColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];
        _scanRect = CGRectMake(60, 100, frame.size.width - 2 * 60, frame.size.width - 2 * 60);
...
//
//    UILabel * descLabel  = [[UILabel alloc] initWithFrame:CGRectMake(0, self.scanRect.origin.y + self.scanRect.size.height + 10, rect.size.width, 20)];
...
//
//    UIButton * myCode = [[UIButton alloc] initWithFrame:CGRectMake(0, self.scanRect.origin.y + self.scanRect.size.height + 10 + 20 + 10, rect.size.width, 20)];
...
- (void)drawScanRect{
    ...
    {
        //设置非识别区域颜色
        const CGFloat *components = CGColorGetComponents(self.backAreaColor.CGColor);
...
- (void)startScan{
    ...
    [UIView animateWithDuration:3.0 animations:^{
        weakSelf.scanLine.frame = CGRectMake(weakSelf.scanRect.origin.x, weakSelf.scanRect.origin.y + weakSelf.scanRect.size.height - 2, weakSelf.scanRect.size.width, 2);
    } completion:^(BOOL finished){
        if (finished == YES) {
            weakSelf.scanLine.frame = CGRectMake(weakSelf.scanRect.origin.x, weakSelf.scanRect.origin.y, weakSelf.scanRect.size.width, 2);
...
- (void)handlingResultsOfScan{
    self.handlingView.center = CGPointMake(self.frame.size.width/2.0, self.scanRect.origin.y + self.scanRect.size.height/2);
...
- (UIImageView *)scanLine{
    if (_scanLine == nil) {
        _scanLine = [[UIImageView alloc] initWithFrame:CGRectMake(self.scanRect.origin.x, self.scanRect.origin.y, self.scanRect.size.width, 4)];
...
- (UIView *)handlingView{
    ...
    if (_handlingView == nil) {
        _handlingView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.scanRect.size.width, 40 + 40)];
