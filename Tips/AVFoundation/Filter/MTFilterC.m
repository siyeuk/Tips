...
#import "MTAVWriteInput.h"

#define KMaxDurationOfVideo  15.0 //录制最大时长 s

@interface MTFilterC ()<MTAVCaptureSessionDelegate, MTAVWriteInputDelegate>
{
...
@property (nonatomic, strong) MTAVWriteInput *avWriterInput;  //音视频写入输出文件
...
- (MTAVWriteInput *)avWriterInput {
    if (!_avWriterInput) {
        _avWriterInput = [[MTAVWriteInput alloc] init];
        _avWriterInput.videoSize = CGSizeMake(MT_kScreenWidth*0.8, MT_kScreenHeight*0.8);
...
- (void)takePicture:(UITapGestureRecognizer *)tap {
    ...
    NSLog(@"拍照结束");
    MTEditImageC * editViewController = [[MTEditImageC alloc] init];
...
- (void)recordVideo:(UILongPressGestureRecognizer *)longPress {
    switch (longPress.state) {
        case UIGestureRecognizerStateBegan:{
            ...
            //开始录制视频
            [self.avWriterInput startWritingToOutputFileAtPath:outputVideoFielPath fileType:MTAVWriterFileTypeVideo deviceOrientation:self.avCaptureSession.shootingOrientation];
...
#pragma mark - MTAVCaptureSessionDelegate 音视频实时输出代理
//实时输出视频样本
...
#pragma mark - MTAVWriteInputDelegate 音视频写入完成
//音视频写入完成
- (void)writerInput:(MTAVWriteInput *)writerInput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL error:(NSError *)error {
    [self.avCaptureSession stopRunning];
    NSLog(@"结束录制");
    MTEditVideoC * editViewController = [[MTEditVideoC alloc] init];
