...
@interface MTDrawBezierPath : UIBezierPath
@property (nonatomic, strong) UIColor *color; //曲线颜色
@end
@implementation MTDrawBezierPath @end
...
/// 笔画
@property (nonatomic, strong) NSMutableArray <MTDrawBezierPath *>*lineArray;
/// 图层
@property (nonatomic, strong) NSMutableArray <CAShapeLayer *>*layerArray;
/// 删除的笔画
@property (nonatomic, strong) NSMutableArray <MTDrawBezierPath *>*deleteLineArray;
/// 删除的图层
@property (nonatomic, strong) NSMutableArray <CAShapeLayer *>*deleteLayerArray;

@end

@implementation MTDrawV
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    if ([event allTouches].count == 1) {
        ...
        MTDrawBezierPath *path = [MTDrawBezierPath new];
        //2、移动画笔
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    if (_isBegan || _isWork) {
        UITouch *touch = [touches anyObject];
        CGPoint point = [touch locationInView:self];
        MTDrawBezierPath *path = self.lineArray.lastObject;
...
//创建线条图层
- (CAShapeLayer *)createShapeLayer:(MTDrawBezierPath *)path {
...
- (void)setData:(NSDictionary *)data {
    ...
    if (lineArray.count) {
        for (MTDrawBezierPath *path in lineArray) {
            CAShapeLayer *slayer = [self createShapeLayer:path];
            [self.layer addSublayer:slayer];
            [self.layerArray addObject:slayer];
        }
