...
@interface MTEditTextV ()<UITextViewDelegate>
{
...
- (void)colorSelectionView:(CGFloat)keyboardHeight {
    ...
    for (int i = 0; i < count; i++) {
        UIButton * colorBtn = [[UIButton alloc] initWithFrame:CGRectMake(space + (itemSize.width + space)*i, self.mt_height - keyboardHeight - 20 - 20, itemSize.width, itemSize.height)];
...
- (UIButton *)doneEditBtn {
    if (_doneEditBtn == nil) {
        _doneEditBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.mt_width - 50 - 15, 30, 40, 30)];
...
- (UILabel *)copyTextView:(UITextView *)textView {
    MTPaddingLabel *label = [[MTPaddingLabel alloc] initWithFrame:textView.bounds];
...
-(void)textViewDidChange:(UITextView *)textView{
    //最大高度
    CGFloat maxHeight = self.mt_height - 100 - _keyboardHeight - 20 - 20 - 20;
    CGSize constraintSize = CGSizeMake(self.mt_width - 2*30, MAXFLOAT);
    CGSize size = [textView sizeThatFits:constraintSize];
    if (size.height >= maxHeight) {
        textView.mt_y = 100 - (size.height - maxHeight);
    } else {
        textView.mt_y = 100;
    }
    textView.mt_height = size.height;
    textView.mt_width = size.width;
}
