...
- (void)setMaskRect:(CGRect)maskRect animated:(BOOL)animated {
    ...
    [self removeAnimationForKey:@"MT_maskLayer_opacityAnimate"];
    if (animated) {
        ...
        animate.toValue = @(1.0);
        self.path = mPath;
        [self addAnimation:animate forKey:@"MT_maskLayer_opacityAnimate"];
    } else {
        self.path = mPath;
    }
}
...
/// 网格层
@interface MTGridLayer : CAShapeLayer
///网格区域 默认CGRectZero
...
@class MTResizeControl;
@protocol MTResizeControlDelegate <NSObject>
/// 开始
- (void)resizeConrolDidBeginResizing:(MTResizeControl *)resizeConrol;
- (void)resizeConrolDidResizing:(MTResizeControl *)resizeConrol;
- (void)resizeConrolDidEndResizing:(MTResizeControl *)resizeConrol;
@end
/// 网格四边和四角控制大小的透明视图
@interface MTResizeControl : UIView
@property (weak, nonatomic) id<MTResizeControlDelegate> delegate;
...
@end
@implementation MTResizeControl
...
//网格视图
@interface MTGridV ()<MTResizeControlDelegate>
@property (nonatomic, strong) MTGridLayer *gridLayer; //网格层
@property (nonatomic, strong) MTGridMaskLayer *gridMaskLayer; // 半透明遮罩层
@property (nonatomic, assign) CGRect initialRect; //高亮网格框的初始区域
//四个角
@property (nonatomic, strong) MTResizeControl *topLeftCornerView;
@property (nonatomic, strong) MTResizeControl *topRightCornerView;
@property (nonatomic, strong) MTResizeControl *bottomLeftCornerView;
@property (nonatomic, strong) MTResizeControl *bottomRightCornerView;
//四条边
@property (nonatomic, strong) MTResizeControl *topEdgeView;
@property (nonatomic, strong) MTResizeControl *leftEdgeView;
@property (nonatomic, strong) MTResizeControl *bottomEdgeView;
@property (nonatomic, strong) MTResizeControl *rightEdgeView;

@end

@implementation MTGridV
...
- (void)enableCornerViewUserInteraction:(UIView *)view {
    for (UIView *control in self.subviews) {
        if ([control isKindOfClass:[MTResizeControl class]]) {
            if (view) {
                if (control == view) {
...
- (MTResizeControl *)createResizeControl {
    MTResizeControl *control = [[MTResizeControl alloc] initWithFrame:(CGRect){CGPointZero, CGSizeMake(kSLControlWidth, kSLControlWidth)}];
...
#pragma mark - Getter
- (MTGridMaskLayer *)gridMaskLayer {
    if (!_gridMaskLayer) {
        _gridMaskLayer = [[MTGridMaskLayer alloc] init];
...
- (MTGridLayer *)gridLayer {
    if (!_gridLayer) {
        _gridLayer = [[MTGridLayer alloc] init];
...
//返回正在调整网格大小时的网格区域
- (CGRect)cropRectMakeWithResizeControlView:(MTResizeControl *)resizeControlView {
    CGRect rect = self.gridRect;
...
#pragma mark - MTResizeControlDelegate
- (void)resizeConrolDidBeginResizing:(MTResizeControl *)resizeConrol {
