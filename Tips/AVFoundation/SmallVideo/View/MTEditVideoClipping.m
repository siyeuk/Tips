...
typedef struct MTVideoClippingState MTVideoClippingState;

CG_INLINE MTVideoClippingState
MTVideoClippingStateMake(BOOL position, CGFloat value, UIGestureRecognizerState state)
{
    MTVideoClippingState videoClippingState;
...
/// 视频编辑裁剪选择框
@interface  MTVideoClippingBox : UIView
...
@property (nonatomic, copy) void(^changeClippingRange)(MTVideoClippingState clippingState);  //改变选择的裁剪区域
@end
@implementation MTVideoClippingBox
...
- (void)drawRect:(CGRect)rect {
    ...
    UIRectFill(rect);
    CGRect holeRectIntersection = CGRectIntersection(CGRectMake(self.leftTime.mt_x+10, 0, self.rightTime.mt_x - (self.leftTime.mt_x+10), rect.size.height), rect);
...
- (void)setFrame:(CGRect)frame{
    ...
    self.boxInside.frame = CGRectMake(0, 0, self.mt_width, self.mt_height);
    self.leftTime.frame = CGRectMake(-10, 0, 10, self.mt_height);
    self.rightTime.frame = CGRectMake(self.mt_width, 0, 10, self.mt_height);
}
#pragma mark - Getter
- (UIView *)boxInside {
    if (!_boxInside) {
        _boxInside = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.mt_width, self.mt_height)];
...
- (UILabel *)leftTime {
    if (!_leftTime) {
        _leftTime = [[UILabel alloc] initWithFrame:CGRectMake(-10, 0, 10, self.mt_height)];
        _leftTime.text = @"||";
...
- (UILabel *)rightTime {
    if (!_rightTime) {
        _rightTime = [[UILabel alloc] initWithFrame:CGRectMake(self.mt_width, 0, 10, self.mt_height)];
        _rightTime.text = @"||";
        _rightTime.tag = 1;
...
- (void)changeClippingRange:(UIPanGestureRecognizer *)pan {
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        self.changeClippingRange(MTVideoClippingStateMake(pan.view.tag, (pan.view.tag == 0 ? self.boxInside.mt_x/self.mt_width : self.rightTime.mt_x/self.mt_width), UIGestureRecognizerStateBegan));
    } else if (pan.state == UIGestureRecognizerStateChanged ) {
        if (pan.view == self.leftTime ) {
            //裁剪的视频时长必须>=1秒
            if (pan.view.mt_x + transP.x < -10 || (self.rightTime.mt_x - self.boxInside.mt_x -  transP.x)/self.mt_width*self.totalDuration < 1) {
                NSLog(@"视频超出了裁剪范围或裁剪后的时长小于1s");
            }else {
                self.boxInside.mt_x = self.boxInside.mt_x + transP.x;
                self.boxInside.mt_width = self.boxInside.mt_width - transP.x;
                pan.view.center = CGPointMake(pan.view.center.x + transP.x, pan.view.center.y);
            }
        }else if (pan.view == self.rightTime) {
            if (pan.view.mt_x + transP.x > self.mt_width || (self.rightTime.mt_x + transP.x - self.boxInside.mt_x)/self.mt_width*self.totalDuration < 1) {
                NSLog(@"视频超出了裁剪范围或裁剪后的时长小于1s");
            }else {
                self.boxInside.mt_width = self.boxInside.mt_width + transP.x;
                pan.view.center = CGPointMake(pan.view.center.x + transP.x, pan.view.center.y);
        ...
        self.changeClippingRange(MTVideoClippingStateMake(pan.view.tag, (pan.view.tag == 0 ? self.boxInside.mt_x/self.mt_width : self.rightTime.mt_x/self.mt_width), UIGestureRecognizerStateChanged));
        ...
        if (pan.view == self.leftTime) {
            ...
            if (pan.view.mt_x  > self.mt_width) {
                pan.view.mt_x = self.mt_width;
            }
        }
        self.changeClippingRange(MTVideoClippingStateMake(pan.view.tag, (pan.view.tag == 0 ? self.boxInside.mt_x/self.mt_width : self.rightTime.mt_x/self.mt_width), UIGestureRecognizerStateEnded));
    }
}
@end


@interface MTEditVideoClipping ()
/// 视频帧图片解析器
...
/// 视频裁剪框
@property (nonatomic, strong) MTVideoClippingBox *clippingBox; //剪辑选择框
...
@implementation MTEditVideoClipping
...
- (UIView *)contentView {
    if (!_contentView) {
        _contentView = [[UIView alloc] initWithFrame:CGRectMake(40, 10, self.mt_width - 40 * 2, 50)];
...
- (UIView *)clippingBox {
    if (!_clippingBox) {
        _clippingBox = [[MTVideoClippingBox alloc] initWithFrame:self.contentView.frame];
        __weak typeof(self) weakSelf = self;
        _clippingBox.changeClippingRange = ^(MTVideoClippingState clippingState) {
...
- (UIButton *)cancleBtn {
    if (_cancleBtn == nil) {
        _cancleBtn = [[UIButton alloc] initWithFrame:CGRectMake(10, self.mt_height - 10 - 30, 30, 30)];
...
- (UIButton *)doneBtn {
    if (_doneBtn == nil) {
        _doneBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.mt_width - 30 - 10, self.mt_height - 10 - 30, 30, 30)];
...
- (void)generateImage {
    ...
    if (maxImageCount * maximumSize.width/[UIScreen mainScreen].scale < self.contentView.frame.size.width) {
        ...
        self.contentView.mt_width = maxImageCount * maximumSize.width/[UIScreen mainScreen].scale;
        self.contentView.mt_x = (self.mt_width - self.contentView.mt_width)/2.0;
        self.clippingBox.frame = self.contentView.frame;
        imageViewSize = CGSizeMake(self.contentView.mt_height*maximumSize.width/maximumSize.height, self.contentView.mt_height);
    }else {
        imageViewSize = CGSizeMake(self.contentView.mt_width/maxImageCount, self.contentView.mt_height);
    }
