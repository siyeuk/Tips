...
/// 马赛克点元素
@interface MTMosaicPointElement : NSObject
@property (nonatomic, assign) CGRect rect;
@property (nonatomic, strong) UIColor *color;
@property (nonatomic, copy) NSString *imageName;
@end
@implementation MTMosaicPointElement @end

/// 马赛克线条 遮罩层  线
@interface MTMosaicLineLayer : CALayer
@property (nonatomic, strong) NSMutableArray <MTMosaicPointElement *>*elementArray;
@end
@implementation MTMosaicLineLayer
- (instancetype)init {
    self = [super init];
    if (self) {
...
- (void)drawInContext:(CGContextRef)context {
    ...
    for (NSInteger i=0; i<self.elementArray.count; i++) {
        MTMosaicPointElement *blur = self.elementArray[i];
...
/** 图层 */
@property (nonatomic, strong) NSMutableArray <MTMosaicLineLayer *>*layerArray;
/** 已显示坐标 */
@property (nonatomic, strong) NSMutableArray <NSValue *>*frameArray;

@end

@implementation MTMosaicV
...
- (void)customInit {
    ...
    _mosaicType = MTMosaicTypeSquare;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    if (touches.allObjects.count == 1) {
        ...
        //2、创建LFSplashBlur
        if (self.mosaicType == MTMosaicTypeSquare) {
            ...
            if (![self.frameArray containsObject:value]) {
                ...
                MTMosaicPointElement *blur = [MTMosaicPointElement new];
                ...
                MTMosaicLineLayer *layer = [MTMosaicLineLayer layer];
        ...
        } else if (self.mosaicType == MTMosaicTypePaintbrush) {
            MTMosaicPointElement  *blur = [ MTMosaicPointElement  new];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    if (_isBegan || _isWork) {
        ...
        /** 获取上一个对象坐标判断是否重叠 */
        MTMosaicLineLayer *layer = self.layerArray.lastObject;
        MTMosaicPointElement *prevBlur = layer.elementArray.lastObject;
...
- (void)goBack {
    ...
    MTMosaicLineLayer *layer = self.layerArray.lastObject;
    if ([layer.elementArray.firstObject isMemberOfClass:[MTMosaicPointElement class]]) {
        for (MTMosaicPointElement *blur in layer.elementArray) {
...
- (NSDictionary *)data {
    if (self.layerArray.count) {
        ...
        for (MTMosaicLineLayer *layer in self.layerArray) {
