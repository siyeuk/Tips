...
@interface MTShotC ()<MTAVCaptureToolDelegate>{
    dispatch_source_t _gcdTimer; //计时器
    NSTimeInterval _durationOfVideo;  //录制视频的时长
}
@property (nonatomic, strong) MTAVCaptureTool *avCaptureTool; //摄像头采集工具
...
#pragma mark - Getter
- (MTAVCaptureTool *)avCaptureTool {
    if (_avCaptureTool == nil) {
        _avCaptureTool = [[MTAVCaptureTool alloc] init];
        _avCaptureTool.preview = self.captureView;
        _avCaptureTool.delegate = self;
        _avCaptureTool.videoSize = CGSizeMake(MT_kScreenWidth*0.8, MT_kScreenHeight*0.8);
    }
...
- (void)recordVideo:(UILongPressGestureRecognizer *)longPress {
    switch (longPress.state) {
        case UIGestureRecognizerStateBegan:{
            ...
            //开始录制视频
            [self.avCaptureTool startRecordVideoToOutputFileAtPath:outputVideoFielPath recordType:MTAVCaptureTypeAv];
        }
            NSLog(@"开始摄像");
            break;
...
#pragma mark - MTAVCaptureToolDelegate  图片、音视频输出代理
//图片输出完成
- (void)captureTool:(MTAVCaptureTool *)captureTool didOutputPhoto:(UIImage *)image error:(NSError *)error {
    [self.avCaptureTool stopRunning];
    NSLog(@"拍照结束");
...
//音视频输出完成
- (void)captureTool:(MTAVCaptureTool *)captureTool didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL error:(NSError *)error {
