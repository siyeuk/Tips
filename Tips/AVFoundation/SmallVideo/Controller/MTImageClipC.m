...
@interface MTImageClipC ()
// 缩放视图
@property (nonatomic, strong) MTImageZoomV *zoomView;
//网格视图 裁剪框
@property (nonatomic, strong) MTGridV *gridView;
...
- (void)setupUI {
    ...
    self.maxGridRect = CGRectMake(KGridLRMargin, KGridTopMargin, self.view.mt_width - KGridLRMargin * 2, self.view.mt_height - KGridTopMargin - KGridBottomMargin- KBottomMenuHeight);
    
    CGSize newSize = CGSizeMake(self.view.mt_width - 2 * KGridLRMargin, (self.view.mt_width - 2 * KGridLRMargin)*self.image.size.height/self.image.size.width);
    if (newSize.height > self.maxGridRect.size.height) {
        ...
        self.zoomView.mt_size = newSize;
        self.zoomView.mt_y = KGridTopMargin;
        self.zoomView.mt_centerX = self.view.mt_width/2.0;
    }else {
        self.zoomView.mt_size = newSize;
        self.zoomView.center = CGPointMake(self.view.mt_width/2.0, (self.view.mt_height - KBottomMenuHeight)/2.0);
    }
...
- (MTImageZoomV *)zoomView {
    if (!_zoomView) {
        _zoomView = [[MTImageZoomV alloc] initWithFrame:CGRectMake(KGridLRMargin, KGridTopMargin, self.view.mt_width - KGridLRMargin *2,( self.view.mt_width - KGridLRMargin *2)*self.image.size.height/self.image.size.width)];
        _zoomView.mt_centerY = (self.view.mt_height - KBottomMenuHeight)/2.0;
...
- (MTGridV *)gridView {
    if (!_gridView) {
        _gridView = [[MTGridV alloc] initWithFrame:self.view.bounds];
...
- (UIButton *)rotateBtn {
    if (_rotateBtn == nil) {
        _rotateBtn = [[UIButton alloc] initWithFrame:CGRectMake(30, self.view.mt_height - KBottomMenuHeight, 40, 30)];
...
- (UIButton *)cancleClipBtn {
    if (_cancleClipBtn == nil) {
        _cancleClipBtn = [[UIButton alloc] initWithFrame:CGRectMake(30, self.view.mt_height - 30 - 20, 40, 30)];
...
- (UIButton *)recoveryBtn {
    if (_recoveryBtn == nil) {
        ...
        _recoveryBtn.mt_centerX = self.view.mt_width/2.0;
        _recoveryBtn.mt_centerY = self.cancleClipBtn.center.y;
...
- (UIButton *)doneClipBtn {
    if (_doneClipBtn == nil) {
        _doneClipBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.mt_width - 30 - 40, self.view.mt_height - 30 - 20, 40, 30)];
...
- (void)rotateBtnClicked:(id)sender {
    ...
    //计算旋转之后
    CGSize newSize = CGSizeMake(self.view.mt_width - 2 * KGridLRMargin, (self.view.mt_width - 2 * KGridLRMargin)*height/width);
...
- (void)doneClipClicked:(id)sender {
    [self dismissViewControllerAnimated:NO completion:nil];
    UIImage *clipImage = [self.zoomView.imageView mt_imageByViewInRect:[self rectOfGridOnImageByGridRect:_gridView.gridRect]];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mt_ImageClippingComplete" object:nil userInfo:@{@"image" : roImage}];
}

#pragma mark - MTGridVDelegate
//开始调整
...
- (void)gridViewDidEndResizing:(MTGridV *)gridView {
    ...
    [UIView animateWithDuration:0.25
                     ...
                     animations:^{
        CGSize newSize = CGSizeMake(self.view.mt_width - 2 * KGridLRMargin, (self.view.mt_width - 2 * KGridLRMargin)*gridView.gridRect.size.height/gridView.gridRect.size.width);
        if (newSize.height > self.gridView.maxGridRect.size.height) {
        ...
        [self.zoomView setZoomScale:self.zoomView.zoomScale];
        // 调整contentOffset
        CGFloat zoomScale = self.zoomView.mt_width/gridView.gridRect.size.width;
