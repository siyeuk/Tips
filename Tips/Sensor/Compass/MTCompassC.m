...
#import "MTScaleView.h"

@interface MTCompassC ()<CLLocationManagerDelegate>{
...
- (void)setupUI{
    _scaleView = [[MTScaleView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width - 30, self.view.frame.size.width - 30)];
    _scaleView.center = CGPointMake(self.view.frame.size.width/2, self.view.frame.size.height/3);
    _scaleView.backgroundColor = [UIColor blackColor];
    [self.view addSubview:_scaleView];
    
    _angleLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.view.frame.size.width/2 - 100, _scaleView.frame.size.height + _scaleView.frame.origin.y, 100, 100)];
...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations{
    ...
    //维纬度
    NSString * latitudeStr = [NSString stringWithFormat:@"%3.2f",_currLocation.coordinate.latitude];
    //经度
    NSString * longitudeStr  = [NSString stringWithFormat:@"%3.2f",_currLocation.coordinate.longitude];
    //高度
    NSString * altitudeStr  = [NSString stringWithFormat:@"%3.2f",_currLocation.altitude];
...
-(void)locationManager:(CLLocationManager *)manager didUpdateHeading:(CLHeading *)newHeading{
    ...
    if (newHeading.headingAccuracy>0){
        ...
        //旋转变换
        [_scaleView resetDirection:heading];
